...
import { FileText, Sheet, Plus, Trash2, Pencil, LogOut, Clock } from "lucide-react";
      ...
      <header className="sticky top-0 z-40 border-b border-border/80 bg-background/80 backdrop-blur-md">
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="flex gap-6 text-sm">
            {NAV_TABS.map((tab) => (
              <Link key={tab.href} href={tab.href} className={`border-b-2 py-3 transition-colors ${tab.href === "/dashboard" ? "border-accent text-foreground" : "border-transparent text-muted hover:text-foreground"}`}>
                {tab.label}
              </Link>
            ))}
          </div>
